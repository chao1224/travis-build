...
#    end
  ...
  describe 'osx' do
    it 'installs' do
      data[:config][:os] = 'osx'
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/mdk.pkg\" -fL http://download.mono-project.com/archive/mdk-latest.pkg", timing: true, assert: true, echo: true]
    ...
    it 'installs dotnet' do
      ...
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/dotnet.pkg\" -fL https://dotnetcli.azureedge.net/dotnet/preview/Installers/1.0.0-preview2-003121/dotnet-dev-osx-x64.1.0.0-preview2-003121.pkg", timing: true, assert: true, echo: true]
    ...
    it 'selects alpha' do
      data[:config][:os] = 'osx'
      data[:config][:mono] = 'alpha'
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/mdk.pkg\" -fL http://download.mono-project.com/archive/mdk-latest-alpha.pkg", timing: true, assert: true, echo: true]
    end

    it 'selects beta' do
      data[:config][:os] = 'osx'
      data[:config][:mono] = 'beta'
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/mdk.pkg\" -fL http://download.mono-project.com/archive/mdk-latest-beta.pkg", timing: true, assert: true, echo: true]
    end

    it 'selects weekly' do
      data[:config][:os] = 'osx'
      data[:config][:mono] = 'weekly'
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/mdk.pkg\" -fL http://download.mono-project.com/archive/mdk-latest-weekly.pkg", timing: true, assert: true, echo: true]
    end

    it 'selects 4.0.1' do
      data[:config][:os] = 'osx'
      data[:config][:mono] = '4.0.1'
      should include_sexp [:cmd, "curl --retry 5 -o \"/tmp/mdk.pkg\" -fL http://download.mono-project.com/archive/4.0.1/macos-10-x86/MonoFramework-MDK-4.0.1.macos10.xamarin.x86.pkg", timing: true, assert: true, echo: true]
    end
  end
end
